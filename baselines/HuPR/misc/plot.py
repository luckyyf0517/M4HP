...
def plotHumanPose(batch_joints, cfg, visDir, imageIdx, bbox=None, upsamplingSize=(256, 256), nrow=8, padding=2, truth=False):
    for j in range(len(batch_joints)):
        ...
        rgbPath = os.path.join(cfg.TEST.plotImgDir, 'seq_%04d'%int(namestr[:4]), 'kinect/imgs', '%04d.jpg'%int(namestr[-4:]))
        ...
        joints_edges = [
            [(int(joints[0][0]), int(joints[0][1])), (int(joints[1][0]), int(joints[1][1]))],
