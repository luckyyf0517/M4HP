...
def run_train(args, train_dataloader, val_dataloader, fusiuon_model, smpl, mesh_sampler, start_epoch):
    ...
    # TIMESTAMP = "{0:%Y-%m-%dT%H-%M-%S/}".format(datetime.datetime.now())  # display time
    loss_manager_train = SummaryWriter(args.output_dir + '/loss/train/')
    loss_manager_eval = SummaryWriter(args.output_dir + '/loss/eval/')
    ...
    for iteration, data_dict in enumerate(train_dataloader):
        ...
        # add gt into data_dict
        data_dict['gt_vertices'] = gt_vertices
        data_dict['gt_vertices_sub'] = gt_vertices_sub
        data_dict['gt_vertices_sub2'] = gt_vertices_sub2
        data_dict['gt_3d_joints'] = gt_3d_joints
        ...
        # use diffusion loss to optimize the model only
        ...
        loss_manager_train.add_scalar("diffusion_loss", diffusion_loss.item(), iteration)
...
def main(args):
    ...
    _model, smpl, mesh_sampler, start_epoch = build_model(args, logger)
...
def parse_args():
    ...
    parser.add_argument("--diffusion_loss_weight", default=1000.0, type=float)
