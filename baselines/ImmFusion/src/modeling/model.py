...
from src.modeling.diffusion_transformer import DiT, TimestepEmbedder
from src.modeling.diffusion.gaussian_diffusion import GaussianDiffusion, ModelMeanType
...
class DiffusionFusion(AdaptiveFusion):
    def __init__(self, args, backbone, trans_encoder, diffusion):
        super(DiffusionFusion, self).__init__(args, backbone, trans_encoder)
        self.diffusion: GaussianDiffusion = diffusion['diffusion']
        
        # using transformer blocks as diffusion prediction model     
        self.x_embedder = nn.Linear(in_features=3, out_features=2051)
        w = self.x_embedder.weight.data
        nn.init.xavier_uniform_(w.view([w.shape[0], -1]))
        nn.init.constant_(self.x_embedder.bias, 0)

        self.t_embedder = TimestepEmbedder(hidden_size=2051)  
    ...
    def forward(self, args, data_dict, smpl, mesh_sampler, meta_masks=None, is_train=False):
        ...
        gt_vertices = gt_vertices.expand(batch_size, -1, -1)
        ...
        # concatinate global and local features
        global_feats = torch.cat(global_feats, dim=1)   # [b, 2, 2048]
        local_feats = torch.cat(local_feats, dim=1) # [b, 98, 2051]
        if not args.wo_GIM:
            # integrate global features
            global_feats = self.GIM(global_feats)
        
        # expand global features
        fusion_feat = torch.sum(global_feats, dim=1, keepdim=True).expand(-1, ref_vertices.shape[-2], -1)   # [b, 677, 2048]
        # concatinate global features and 3d mesh template
        features = torch.cat([ref_vertices, fusion_feat], dim=2)   # [b, 677, 2051]
        
        if not args.wo_local_feat:
            # prepare input tokens including joint/vertex queries and local features
            features = torch.cat([features, local_feats], dim=1)    # [b, 775, 2051]
        ...
        # """
        # Diffusion training (To this end, the features are fixed by frozen layers.)
        # """
        pred_dict = {}
        # create time stamps
        t = torch.randint(0, self.diffusion.num_timesteps, (features.shape[0],), device=args.device)
        # corrupt template mesh
        x_start = gt_vertices
        ...
        # if args.train: 
            if self.diffusion.model_mean_type == ModelMeanType.START_X: 
                pred_vertices = self.diffusion_model(x_t, t, features)
            else: 
                pred_noise = self.diffusion_model(x_t, t, features)
                pred_dict['diffusion_loss'] = F.mse_loss(pred_noise, noise)
                pred_vertices = self.diffusion._predict_xstart_from_eps(x_t=x_t, t=t, eps=pred_noise)
        else: 
            # sample from random noise
            z = torch.randn_like(x_start, device=args.device)
            pred_vertices = self.diffusion.ddim_sample_loop(
                self.diffusion_model, z.shape, z, clip_denoised=False, 
                model_kwargs={'c': features}, progress=True, device=args.device
            )
        
        pred_3d_joints = pred_vertices[:, :num_joints, :]
        pred_vertices_sub2 = pred_vertices[:, num_joints: num_joints + num_verts, :]
    ...
    def diffusion_model(self, x, t, c): 
        # act as the diffusion model
        # x: [b, 677, 3]
        # t: [b,]
        # c: [b, 775, 2051]
        x = self.x_embedder(x)  # [b, 677, 2051]
        t = self.t_embedder(t)  # [b, 2051]
        c = c + t.unsqueeze(1)  # [b, 775, 2051]
        c[:, :677, :] = c[:, :677, :] + x
        out = self.trans_encoder(c) # [b, 677, 3]
        return out[:, :677, :]
